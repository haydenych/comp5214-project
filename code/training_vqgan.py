...
class TrainVQGAN:
    def __init__(self, args):
        # self.vqgan = nn.DataParallel(VQGAN(args)).to(device=args.device)

        self.vqgan = VQGAN(args).to(device=args.device)
        # self.discriminator = nn.DataParallel(Discriminator(args)).to(device=args.device)

        self.discriminator = Discriminator(args).to(device='cuda:0')
